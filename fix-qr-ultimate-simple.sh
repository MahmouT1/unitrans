...
echo "🔧 إصلاح QR Code Generation - الحل النهائي البسيط"
echo "=============================================="
...
grep -A 5 -B 5 "generate-qr" backend-new/routes/students.js

echo ""
echo "🔧 2️⃣ إصلاح students/generate-qr API مباشرة:"
echo "========================================="

# Create a simple fix for the generate-qr endpoint
cat > /tmp/qr_ultimate_fix.js << 'EOF'
// Generate QR Code for existing student
router.post('/generate-qr', async (req, res) => {
  try {
    const { email, studentId } = req.body;
    
    console.log('🔗 QR Generation request:', { email, studentId });
...
router.post('\''/generate-qr'\'', async (req, res) => {\
  try {\
    const { email, studentId } = req.body;\
    \
    console.log('\''🔗 QR Generation request:'\'' + JSON.stringify({ email, studentId }));\
    \
    // Accept both email and studentId\
    let query = {};\
    if (email) {\
