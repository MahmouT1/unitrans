...
const SupervisorDashboard = () => {
  ...
  // Auto-refresh live attendance when shift is active
  useEffect(() => {
    let interval;
    if (currentShift && currentShift.status === 'active') {
      loadLiveAttendance();
      interval = setInterval(loadLiveAttendance, 5000); // Refresh every 5 seconds
    }
    return () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          loadLiveAttendance(); // Refresh live attendance
        } else {
          ...
          <div style={{
            ...
            {showAttendanceDetails && currentShift && liveAttendance.length > 0 && (
              <div style={{
                ...
                <h3 style={{ margin: '0 0 10px 0', color: '#333' }}>Recent Attendance Records</h3>
                <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#e9ecef' }}>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #dee2e6' }}>Student</th>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #dee2e6' }}>Time</th>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #dee2e6' }}>Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {liveAttendance.filter(record => {
                        return record.studentData && record.studentData.fullName;
                      }).map((record, index) => (
                        <tr key={index}>
                          <td style={{ padding: '8px', border: '1px solid #dee2e6' }}>
                            {record.studentData?.fullName || 'Unknown'}
                          </td>
                          <td style={{ padding: '8px', border: '1px solid #dee2e6' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#10B981' }}>{attendanceCount}</div>
                <div style={{ color: '#666' }}>Total Attendance</div>
              </div>
              <div style={{ textAlign: 'center' }}>
