...
echo "🔄 استعادة التصميم الأصلي لصفحة student portal"
echo "=============================================="
...
echo ""
echo "🎨 استعادة التصميم الأصلي مع إصلاح localStorage:"
echo "=============================================="

# استعادة التصميم الأصلي مع إصلاح localStorage keys فقط
...
export default function StudentPortal() {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      // 🔧 البحث في جميع localStorage keys المحتملة (FIX للمشكلة)
      ...
      if (!token || !userData) {
        ...
        router.push('/login'); // 🔧 تم تغييرها من /auth إلى /login
        return;
      }
      ...
      try {
        ...
        // 🔧 تم إزالة الـ API call المُشكِل (/api/students/profile-simple)
        // والاعتماد على localStorage فقط
        if (studentData) {
          try {
            setStudent(JSON.parse(studentData));
          } catch (error) {
            console.log('Error parsing student data:', error);
          }
        }
  ...
  const handleLogout = () => {
    ...
    // 🔧 تنظيف جميع localStorage keys المحتملة
...
echo "✅ تم استعادة التصميم الأصلي"
...
if [ $BUILD_STATUS -eq 0 ]; then
    ...
    echo ""
    echo "🧪 اختبار student portal نهائي:"
    echo "=============================="
...
echo ""
echo "✅ استعادة التصميم الأصلي اكتملت!"
echo "🎨 التصميم الأصلي مُستعاد مع إصلاح localStorage"
echo "🔗 جرب: https://unibus.online/login"
echo "   📧 test@test.com / 123456"
echo "   🎯 الآن سيعمل مع التصميم الأصلي!"