...
echo "🔧 Fix Supervisor Close Shift"
echo "============================"
...
# Update the supervisor dashboard page
...
export default function SupervisorDashboard() {
    ...
    const closeShift = async () => {
        ...
        try {
            ...
            // Use the /:id/close endpoint instead of /close
...
# Create the shifts API route for closing
echo "🔧 Creating shifts close API route..."

mkdir -p app/api/shifts/\[id\]/close

cat > app/api/shifts/\[id\]/close/route.js << 'EOF'
...
export async function PUT(request, { params }) {
    try {
        const { id } = params;
        console.log('Frontend shifts close API - PUT request for shift:', id);
        
        const response = await fetch(`${BACKEND_URL}/api/shifts/${id}/close`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
echo ""
echo "✅ Supervisor close shift fix completed!"
echo "🌍 Test your project at: https://unibus.online/admin/supervisor-dashboard"
echo "📋 The 'CLOSE SHIFT' button should now work correctly"
