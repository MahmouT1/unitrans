...
echo "🔧 Fixing Admin Attendance Page"

cd /home/unitrans/frontend-new

# Create a fixed version of the attendance page
  ...
  const loadActiveShifts = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.success && data.shifts) {
          setActiveShifts(Array.isArray(data.shifts) ? data.shifts : []);
...
echo "✅ Attendance page fix complete!"
