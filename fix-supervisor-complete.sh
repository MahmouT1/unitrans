...
echo "🔄 إصلاح صفحة المشرف بالكامل من الملفات الأصلية..."
...
# Restore the complete original supervisor dashboard with all features
...
const SupervisorDashboard = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.shift) {
          ...
          fetchShiftAttendance(data.shift.id);
        }
      }
    } catch (error) {
  ...
  const fetchShiftAttendance = async (shiftId) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setShiftAttendance(data.attendance || []);
      }
    } catch (error) {
  ...
  const handleQRCodeScanned = async (qrData) => {
    try {
      ...
      if (!currentShift) {
        setScanError('No active shift. Please open a shift first.');
        showNotification('error', 'No Active Shift', 'Please open a shift before scanning QR codes');
        return;
      }

      // Register attendance
      const attendanceResponse = await fetch('/api/shifts/scan', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (attendanceResponse.ok) {
        ...
        showNotification('success', 'Attendance Registered', `تم تسجيل حضور الطالب: ${studentData.name || studentData.studentId}`);
        
        // Refresh shift attendance
        fetchShiftAttendance(currentShift.id);
        
        // Set scanned student for display
        setScannedStudent({
          ...studentData,
        ...
        setScanError(attendanceResult.message || 'Failed to register attendance');
        showNotification('error', 'Registration Failed', attendanceResult.message || 'Failed to register attendance');
      }
  ...
  return (
    <div className="supervisor-dashboard">
      ...
      <div className="shift-management-section">
          ...
          <div className="shift-info">
            {currentShift ? (
              <div>
                ...
                <p><strong>Total Scans:</strong> {shiftAttendance.length}</p>
              </div>
            ) : (
              <div>
        ...
        {/* Live Attendance Records - Only show when shift is open */}
        {currentShift && shiftAttendance.length > 0 && (
          <div style={{ marginTop: '1rem' }}>
            <h4>Live Attendance Records</h4>
            <table className="attendance-table">
              ...
              <tbody>
                {shiftAttendance.map((record, index) => (
                  <tr key={index}>
              ...
              <div className="student-details-content">
                <div className="student-profile">
                  ...
                  <div className="student-basic-info">
                    ...
                    <p><strong>Scan Time:</strong> {scannedStudent.scanTime}</p>
                    <p><strong>Status:</strong> {scannedStudent.status}</p>
                  </div>
                </div>
...
echo "✅ تم إصلاح صفحة المشرف بالكامل!"
...
echo "   ✅ عرض سجل الحضور المباشر داخل الكارت الأخضر"
echo "   ✅ رسالة تأكيد تسجيل الحضور بعد مسح QR"
echo "   ✅ ربط مع صفحة الحضور الرئيسية للأدمين"
echo "   ✅ حفظ سجلات الحضور في قاعدة البيانات"
echo "   ✅ إخفاء الكارت الأخضر عند إغلاق الوردية"
