...
echo "🔧 إصلاح مشكلة PayloadTooLargeError بشكل نهائي"
echo "=========================================="
...
if [ -f "backend-new/server.js" ]; then
    ...
    if grep -q "limit.*100mb" backend-new/server.js; then
        echo "✅ Payload limit موجود (100mb)"
    else
...
app.listen(PORT, () => {
    ...
    console.log(`Payload limit: 200MB`);
    console.log(`Environment: ${process.env.NODE_ENV || 'production'}`);
});
EOF

echo "✅ تم إنشاء server.js جديد مع payload limits صحيحة (200MB)"
...
echo "⏳ انتظار 25 ثانية للتأكد من التشغيل..."
sleep 25
...
echo "   🔧 تم زيادة payload limit إلى 200MB"
