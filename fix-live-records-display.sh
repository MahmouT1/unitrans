...
echo "🔧 Fixing Live Records Display"

cd /home/unitrans/frontend-new

# Backup current file
cp app/admin/attendance/page.js app/admin/attendance/page.js.backup5

# Create enhanced version with proper live records display
...
function AdminAttendancePageContent() {
  ...
  }, [user, selectedDate, selectedSupervisor, showLiveView]);
  ...
  const loadLiveData = async () => {
    try {
      ...
      const response = await fetch(`/api/attendance/all-records?date=${today}`, {
        headers: {
  ...
  const loadLiveAttendanceRecords = async () => {
    try {
      const token = localStorage.getItem('token');
      const today = new Date().toISOString().split('T')[0];
      
      console.log('Loading live attendance records for date:', today);
      
      const response = await fetch(`/api/attendance/all-records?date=${today}&limit=50`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.records)) {
          ...
          const sortedRecords = data.records.sort((a, b) => 
            new Date(b.scanTime) - new Date(a.scanTime)
          );
          setLiveAttendanceRecords(sortedRecords);
          console.log('Live records loaded:', sortedRecords.length);
        } else {
          setLiveAttendanceRecords([]);
          console.log('No live records found');
        }
      } else {
        console.error('Failed to load live attendance records:', response.status);
        setLiveAttendanceRecords([]);
      }
    } catch (error) {
      console.error('Error loading live attendance records:', error);
      setLiveAttendanceRecords([]);
    }
  };
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' }}>
            ...
            <div>
              ...
              <p style={{ margin: '5px 0 0 0', opacity: '0.9', fontSize: '14px' }}>
                Click to view live attendance records
              </p>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0', fontSize: '18px', fontWeight: 'bold' }}>
              📊 Live Attendance Records ({liveAttendanceRecords.length})
            </h3>
            <button
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {Array.isArray(liveAttendanceRecords) && liveAttendanceRecords.length > 0 ? (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {liveAttendanceRecords.map((record, index) => (
                    <tr key={record._id || index} style={{ 
                      borderBottom: '1px solid #e5e7eb',
                      background: index % 2 === 0 ? '#f9fafb' : 'white'
                    }}>
                      <td style={{ padding: '12px', color: '#6b7280', fontWeight: '600' }}>
                        {index + 1}
                      </td>
              ...
              <div style={{ 
                ...
                <div>No live attendance records found</div>
                <div style={{ fontSize: '14px', marginTop: '10px', opacity: '0.7' }}>
                  Records will appear here when students scan their QR codes
                </div>
              </div>
            )}
          </div>
...
echo "✅ Live records display fix complete!"
