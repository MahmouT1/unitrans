...
# Create backup first
cp app/admin/supervisor-dashboard/page.js app/admin/supervisor-dashboard/page.js.backup-$(date +%Y%m%d_%H%M%S)

# Write the complete supervisor dashboard page
  ...
  const handleQRCodeScanned = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification('success', 'Attendance Registered', `Successfully registered attendance for ${result.studentData?.name || 'student'}`);
        
        // Refresh live attendance
        loadLiveAttendance();
        
        // Set scanned student data for display
        setScannedStudent(result.studentData);
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
            <button
              ...
              disabled={isScanning || !currentShift}
              style={{
                background: isScanning ? '#9CA3AF' : 'linear-gradient(135deg, #10B981 0%, #059669 100%)',
                color: 'white',
                border: 'none',
                padding: '12px 24px',
                borderRadius: '8px',
                fontSize: '16px',
                fontWeight: '600',
                cursor: (isScanning || !currentShift) ? 'not-allowed' : 'pointer',
                transition: 'all 0.3s ease'
              }}
            >
            ...
            <button
              ...
              disabled={!isScanning}
              style={{
                background: !isScanning ? '#9CA3AF' : 'linear-gradient(135deg, #EF4444 0%, #DC2626 100%)',
                color: 'white',
                border: 'none',
                padding: '12px 24px',
                borderRadius: '8px',
